...
Prepare SUNRGB-D dataset in emsanet verion - useful to reproduce results
reported in EMSANet or EMSAFormer paper.
Script was copied from v0.5.6 and adapted to work with >= v0.7.0.
...
def main(args=None):
    ...
    parser.add_argument(
        ...
        help="Version of instance annotations to extract. Over time, we have "
             ...
             "extraction (see changelog for v0.6.0 of this package). This "
             "script only supports the 'emsanet' version."
    )
    parser.add_argument(
    ...
    for i, (meta, meta_3d) in tqdm(enumerate(zip(SUNRGBDMetaMat,
        ...
        boxes_3d_path = os.path.join(output_path, split_dir,
                                     SUNRGBDMeta.BOXES_EMSANET_DIR, cam_path,
                                     f'{i:05d}.json')
