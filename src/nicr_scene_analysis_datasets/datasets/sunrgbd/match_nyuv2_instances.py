...
class NYUv2InstancesMatcher:
    ...
    def do_matching(self, verbose=False, dry_run=False):
        ...
        for split in self.splits:
            ...
            nyuv2_image_dir = self.get_nyuv2_path(
                split,
                NYUv2Meta.RGB_DIR
            )
            nyuv2_image_content = sorted(os.listdir(nyuv2_image_dir))

            sun_image_dir = self.get_sunrgbd_path_for_nyuv2(
                split,
                SUNRGBDMeta.RGB_DIR
            )
            sun_image_content = sorted(os.listdir(sun_image_dir))

            nyuv2_instance_dir = self.get_nyuv2_path(
                split,
                NYUv2Meta.INSTANCES_DIR
            )
            ...
            nyuv2_orientation_dir = self.get_nyuv2_path(
                split,
