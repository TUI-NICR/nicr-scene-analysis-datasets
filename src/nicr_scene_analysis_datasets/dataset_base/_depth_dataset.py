...
def depth_compute_stats(
    ...
    dataset.debug_print(
        "Computing mean, std, min, and max for depth images ..."
    )

    def compute_helper1(sample_idx):
        depth = dataset.load('depth', sample_idx)
        if 'raw' == dataset.depth_mode:
    ...
    dataset.debug_print("[1/2] Computing mean, min, and max ...")

    # compute max, min, total sum, number of pixel
    if n_threads == 1:
        for i in tqdm(range(len(dataset)),
                      total=len(dataset),
                      disable=dataset._disable_prints):
    ...
    dataset.debug_print("[2/2] Computing std ...")
...
class DepthDataset(DatasetBase):
    ...
    def depth_compute_stats(
        ...
        return depth_compute_stats(self, n_threads=n_threads, debug=debug)
